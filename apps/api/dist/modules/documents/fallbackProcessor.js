export class FallbackProcessor {
    ...
    simulateOCR(fileBuffer, mimeType, filename) {
...
END DATE: 2024-12-14`;
    ...
    generateActaLiquidacionText(filename) {
...
DATE: ${new Date().toISOString().split('T')[0]}`;
    ...
    enhancedClassification(text, filename) {
    ...
    enhancedExtraction(text, documentType, filename) {
        switch (documentType) {
    ...
    extractContractCertification(text, filename) {
    ...
    calculateEnhancedConfidence(data, documentType) {
        let confidence = 0.6;
        // Increase confidence based on extracted data
        if (data.valorSinIva && data.valorSinIva > 0)
            confidence += 0.1;
        if (data.cliente && data.contratista)
            confidence += 0.1;
        if (data.numeroContrato && data.numeroContrato !== `CT-${Date.now()}`)
            confidence += 0.1;
