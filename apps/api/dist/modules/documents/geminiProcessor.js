...
PERIODO: Enero 2025 - Julio 2025`
    ...
    async classifyWithGemini(text, filename) {
        try {
            const prompt = `
    ...
    async extractWithGemini(text, documentType) {
        try {
            const schemas = {
    ...
    postProcessContractCertification(data) {
    ...
    calculateConfidence(data, documentType) {
    ...
    keywordClassification(text, filename) {
    ...
    fallbackExtraction(text, documentType) {
    ...
    fallbackProcessing(text, filename) {
